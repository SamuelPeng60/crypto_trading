...
function migrate(db: Database.Database) {
  ...
  // Migration 15: Add supertrend_macd strategy type to CHECK constraint
  try {
    const row = db.prepare(
      "SELECT sql FROM sqlite_master WHERE type='table' AND name='strategies'"
    ).get() as { sql: string } | undefined
    if (row && !row.sql.includes('supertrend_macd')) {
      db.pragma('foreign_keys = OFF')
      try {
        db.exec(`CREATE TABLE strategies_v5 (
          id          INTEGER PRIMARY KEY AUTOINCREMENT,
          name        TEXT NOT NULL,
          type        TEXT NOT NULL CHECK(type IN ('ma_cross','rsi','grid','supertrend','vwap_bb_rsi','ema_ribbon_st','macd_bb_squeeze','adaptive_combo','ma_consolidation_breakout','supertrend_macd')),
          symbol      TEXT NOT NULL,
          params      TEXT NOT NULL,
          is_active   INTEGER NOT NULL DEFAULT 0,
          created_at  TEXT NOT NULL DEFAULT (datetime('now')),
        ...
        db.exec(`INSERT INTO strategies_v5 SELECT id, name, type, symbol, params, is_active, created_at, updated_at, session_id, last_signal, mode FROM strategies`)
        db.exec(`DROP TABLE strategies`)
        db.exec(`ALTER TABLE strategies_v5 RENAME TO strategies`)
      } finally {
        db.pragma('foreign_keys = ON')
      }
    }

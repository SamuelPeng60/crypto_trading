...
import { backtestVwapBbRsi, backtestAdaptiveCombo, backtestSupertrend } from '../lib/backtest.js'
...
const results = { vwap: [], adaptive: [], st: [] }
...
for (const p of PERIODS) {
  ...
  const rv = backtestVwapBbRsi(klines, vwapParams, CAPITAL)
  const ra = backtestAdaptiveCombo(klines, adaptiveParams, CAPITAL)
  const rs = backtestSupertrend(klines, stParams, CAPITAL)
